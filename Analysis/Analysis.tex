...
Potenzreihen haben der Form $\sum^\infty_0 a_nx^n$.
\paragraph{Der konvergenz radius}
\begin{equation}
...
\end{equation}
wenn $|x|<\rho$ dann konvergiert die Reihe. In diesem gebiet darf man die Reihe ableiten und Integrieren.
\subsubsection{Tips}
\begin{equation}
...
\subsection{Gradient}
\begin{equation}
	graf(f)=\bigtriangledown=
	\begin{bmatrix}
		\frac{\vartheta}{\vartheta x_1}\\
		...\\
		\frac{\vartheta}{x_n}\\
	\end{bmatrix}
\end{equation}
Der gradient zeigt in die richtung der maximalen zuwachsrate von f und seine lange ist gleich der maximalen anderung von f.
\subsection{Rotation}
Fur ein vektorfeld mit komponenten: $v_1,v_2,v_3$.
\begin{equation}
	rot(v)=\bigtriangledown\times v =
	\begin{bmatrix}
		\frac{\vartheta v_3}{\vartheta y} - \frac{\vartheta v_2}{\vartheta z}\\
		\frac{\vartheta v_1}{\vartheta z} - \frac{\vartheta v_3}{\vartheta x}\\
		\frac{\vartheta v_2}{\vartheta x} - \frac{\vartheta v_1}{\vartheta y}
	\end{bmatrix}
\end{equation}
Falls die rot(v)=0 dann ist v Konservativ.
...
\paragraph{Kritischer Punkt:} $p_0 \in \Omega$ f{\"u}r welchen $df(p_0)$ nicht den maximalen Rang besitzt, also falls $Rang(df(p_0)) < \min{n, m}$. Falls $df(p_0) = 0$
\subsection{Hesse matrix}
\begin{equation}
	Hess(f)=
	\begin{bmatrix}
		\frac{\vartheta^2 f}{\vartheta^2x_1^2} & ... & \frac{\vartheta^2 f}{\vartheta^2x_1 \vartheta x_n}\\
		...&...&...\\
		\frac{\vartheta^2 f}{\vartheta^2x_n \vartheta x_1} & ... & \frac{\vartheta^2 f}{\vartheta x_n^2}
	\end{bmatrix}
\end{equation}
...
\subsection{Ohne nebenbedigungen}
\begin{enumerate}
	\item finden df(x)=0
	\item hesse matrix finden ob est max min oder sattelpunkt ist.
\end{enumerate}
\subsection{Mit nebenbedingungen}
\paragraph{Lagrange multiplikator}
g ist die bedingung und f die hauptfunktiom, falls $x_0$ ein max oder min ist dann gilt:
\begin{equation}
	L=f-\lambda g
\end{equation}
\begin{equation}
...
\end{equation}
\begin{enumerate}
	\item finden die Kandidaten im inneren von Gebiet wie vorher
	\item schreibe den Rand in: $\vartheta \Omega = g^{-1}\{0\}$
	\item Stelle die funktion $L=f - \lambda g$
	\item die Kritische punkte der lagrange funktion sind kandidaten wo the ableitung muss gleich 0 sein und g muss gleich null sein.
	\item Eckepunkte sind auch noch kandidate
\end{enumerate}
Mann kann auch aussert lagrange einfach die gerade die um den Gebiet gehen und sie ableiten und gleich 0 stellen.\\
Um die tangential zu der ebene von eine extramastelle von diese ebene zu finden, die ebene z=f(extremestelle)(die ebene ist constant weil es eine extrema stelle ist).
